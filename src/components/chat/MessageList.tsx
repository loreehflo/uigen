...
function getFriendlyToolLabel(toolName: string, args: Record<string, unknown> | undefined, isDone: boolean): string {
  const path = typeof args?.path === "string" ? args.path.split("/").pop() || args.path : undefined;
  const command = typeof args?.command === "string" ? args.command : undefined;

  if (toolName === "str_replace_editor") {
    const verb = (() => {
      switch (command) {
        case "view": return isDone ? "Read" : "Reading";
        case "create": return isDone ? "Created" : "Creating";
        case "str_replace":
        case "insert": return isDone ? "Edited" : "Editing";
        case "undo_edit": return isDone ? "Reverted" : "Reverting";
        default: return isDone ? "Updated" : "Updating";
      }
    })();
  ...
  if (toolName === "file_manager") {
    const verb = (() => {
      switch (command) {
        case "rename": return isDone ? "Renamed" : "Renaming";
        case "delete": return isDone ? "Deleted" : "Deleting";
        default: return isDone ? "Updated" : "Updating";
      }
    })();
    return path ? `${verb} ${path}` : verb;
                      ...
                      {message.parts.map((part, partIndex) => {
                        switch (part.type) {
                          ...
                          case "tool-invocation":
                            ...
                            const label = getFriendlyToolLabel(tool.toolName, tool.args, isDone);
                            return (
